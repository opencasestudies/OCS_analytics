...
cstotalsplot = ggplot(drop_na(pageTotals), aes(x = reorder(code, -ocs_sum), y = ocs_sum, fill = code)) +
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  ylim(c(0,850)) + guides(fill = "none") + geom_text(aes(label = paste0(ocs_sum)), vjust = -.5)
```
...
interactivesplot = ggplot(drop_na(interactiveTotals), 
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  ylim(c(0,50)) + guides(fill = "none") + geom_text(aes(label = paste0(ocs_sum)), vjust = -.5)
```

```{r case_study_sessionsplot}
ggarrange(cstotalsplot, interactivesplot,
          labels = c("A.", "B."), 
          ncol = 1, nrow = 2)
```
