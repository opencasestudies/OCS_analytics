...
dates = ga_data(ocs_property_id,
        date_range = c("2021-01-01", "2021-10-06"),
        metric = c("sessions"),
...
```{r}
cities10 = cities %>% filter(country != "Peru") %>% head(10) %>% 
...
urldf = bind_rows(staticTotals, interactiveTotals)

ggplot(urldf, aes(x = code, y = ocs_sum, fill = interactive)) +
  geom_bar(position = "dodge", stat = "identity") + #scale_fill_viridis_d() + 
  labs(x = "Case Study", y = "Session Count", 
       title = "Number of Sessions by Case Study", fill = "Version") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  ylim(c(0,750)) + scale_fill_manual(labels = c("Static", "Interactive"), values = c("#0032A0", "#D00070"))
```
...
engagedSessions = ga_data(ocs_property_id,
          date_range = c("2021-01-01", "2021-10-6"),
...
ggplot(ses_long, aes(x = as.numeric(week), y = count, color = metric)) +
  geom_line() +
  labs(x = "Week Number", y = "Count", title = "Open Case Studies Weekly Sessions and Engaged Sessions")
```

```{r}
ggplot(engagedSessions, aes(x = as.numeric(week), y = engagementRate, 
                            color = engagementRate)) +
  geom_line() + 
  labs(x = "Week Number", y = "Engagement Rate", 
       title = "Open Case Studies Weekly Engagement Rate") #+ 
