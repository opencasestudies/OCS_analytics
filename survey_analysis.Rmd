...
teach_stud_sum = educatordf %>% select(starts_with("teach_")) %>% 
  select(contains(c("grad", "HS", "middle", "ugrad"))) %>% 
...
#   mutate(across(, ~as.numeric(.))) %>%
#   colSums() %>% as.data.frame() %>% rownames_to_column()
...
ggplot(teach_int_sum, aes(x = topic, y = count, fill = topic)) +
  geom_col() + scale_fill_viridis_d() + 
 # scale_fill_manual(values = c("#5592CC", "#714986", "#4B9B81")) + 
  labs(x = "Subject", y = "Count", title = "Educator Teaching Interests") + 
