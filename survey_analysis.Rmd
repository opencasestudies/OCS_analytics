...
educatordf %<>% 
  mutate(case_studies = gsub(": .*", "", str_remove(case_studies, "Case study about ")))
...
educatordf %<>% 
  mutate(data_types = 
           str_replace(data_types, "Raw data files or original source (ex. a website)", "Raw") %>%
           str_replace("Wrangled data files (as rda files)", "Wrangled RDA") %>%
           str_replace("Wrangled data files (as csv files)", "Wrangled CSV"))
...
studentdf %<>% 
  mutate(studentdf, famil_stats = 
           ifelse(famil_stats == "Never heard about the statistical topics covered", 1, 
           ifelse(famil_stats == "Had heard briefly about the statistical topics covered", 2, 
           ifelse(famil_stats == "Had an understanding about the topics but not as much as the case study covered", 3, 
           ifelse(famil_stats == "Regularly perform the statistical methods covered/already knew all or most of the material", 4, famil_stats)))))
