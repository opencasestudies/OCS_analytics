...
```{r, case_studies}
cs_educators = educatordf %>% select(responder_category, starts_with("looked_at_")) %>%
  select(-looked_at_none)
cs_students = studentdf %>% select(responder_category, starts_with("looked_at_"))
cs_self_learners = self_learnerdf %>% select(responder_category, starts_with("looked_at_"))
looked_at_cs = bind_rows(cs_educators, cs_students, cs_self_learners) %>%
  na_if("NA") 
looked_at_cs = filter(looked_at_cs, rowSums(is.na(looked_at_cs)) != (ncol(looked_at_cs)-1))

looked_at_cs_counts = looked_at_cs %>% mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = -responder_category, names_to = "case_study", values_to = "count") %>%
