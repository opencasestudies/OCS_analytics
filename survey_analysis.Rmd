...
ggplot(consented, aes(x = forcats::fct_infreq(as.factor(responder_category)), fill = responder_category)) +
  ...
  labs(x = "User Category", y = "Count", title = "Count of Survey Responders by Category") + ylim(c(0,12)) + 
...
ggplot(looked_at_cs_counts, aes(x = case_study, y = percent, fill = responder_category)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Case Study", y = "Percent (%)", 
       title = "Percent of Participants Using Each Case Study") +
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
