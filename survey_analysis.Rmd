...
# separate case_studies into boolean columns
...
```{r responder_category}
ggplot(consented, aes(x = forcats::fct_infreq(as.factor(responder_category)), fill = responder_category)) +
  geom_bar(color = "black") + scale_fill_viridis_d() +
  # scale_fill_manual(values = c("#5592CC", "#714986", "#4B9B81")) + 
  labs(x = "User Category", y = "Count", title = "Count of Survey Participants by Category") + ylim(c(0,12)) + 
...
```{r educator_stats}
# educator future teach:
future_teach = educatordf %>% drop_na(future_teach)
num_responses = nrow(future_teach)
future_teach %>% count(future_teach, name = "num_teachers") %>% 
  mutate(percent = 100*num_teachers/num_responses)
```

```{r type_student_taught}
teach_stud_sum = educatordf %>% select(starts_with("teach_")) %>% 
  select(contains(c("ugrad", "grad", "middle", "HS"))) %>% 
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "type", values_to = "count") %>%
  mutate(percent = 100*count/nrow(educatordf)) %>%
    mutate(type = case_when(
    str_detect(type, pattern = "ugrad") == TRUE ~ "Undergraduate",
    str_detect(type, pattern = "grad") == TRUE ~ "Graduate",
    str_detect(type, pattern = "middle") == TRUE ~ "Middle School",
    str_detect(type, pattern = "HS") == TRUE ~ "High School"
    )
  )

leveltaughtplot = ggplot(teach_stud_sum, aes(x = reorder(type, -percent), y = percent, fill = type)) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Student Type", y = "Percent (%)", 
       title = "Types of Students Taught by Educator Participants") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,75))
```

```{r educator_teaching_interests}
teach_int_sum = educatordf %>% select(starts_with("teach_")) %>% 
  select(-contains(c("grad", "HS", "middle", "enjoy"))) %>% 
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "topic", values_to = "count") %>%
  mutate(percent = round(count/nrow(educatordf)*100)) %>% 
  mutate(specific = str_detect(topic, "_spfc_")) %>% 
  mutate(topic = case_when(
    str_detect(topic, pattern = "stats") == TRUE ~ "Statistics",
    str_detect(topic, pattern = "ds") == TRUE ~ "Data Science",
    str_detect(topic, pattern = "PH") == TRUE ~ "Public Health",
    str_detect(topic, pattern = "R") == TRUE ~ "R Programming",
    str_detect(topic, pattern = "tidyverse") == TRUE ~ "Tidyverse"
    )
  ) %>% 
  mutate(name = case_when(
    specific == TRUE ~ paste("Specific", topic, "Topic", sep = "\n"),
    TRUE ~ topic
    )
  )
...
ggplot(teach_int_sum, aes(x = fct_reorder(reorder(as.factor(name), -percent), specific), y = percent, 
                          fill = topic)) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Subject", y = "Percent of Educators (%)", title = "Educator Teaching Interests") + 
  guides(fill = "none") + theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + 
  ylim(c(0,100))
```

```{r courses_taught}
courses_taught_sum = educatordf %>% select(starts_with("would_use_")) %>% 
  select(contains(c("PH", "stats", "math", "DS"))) %>% 
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "subject", values_to = "count") %>%
  mutate(percent = 100*count/nrow(educatordf)) %>%
    mutate(subject = case_when(
    str_detect(subject, pattern = "PH") == TRUE ~ "Public Health",
    str_detect(subject, pattern = "stats") == TRUE ~ "Statistics",
    str_detect(subject, pattern = "math") == TRUE ~ "Mathematics",
    str_detect(subject, pattern = "DS") == TRUE ~ "Data Science"
    )
  )

coursetaughtplot = ggplot(courses_taught_sum, aes(x = reorder(subject, -percent), y = percent, fill = subject)) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Course Subject", y = "Percent (%)", 
       title = "Courses Taught by Educator Participants") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,100))
```

```{r studcoursestaught}
ggarrange(coursetaughtplot, leveltaughtplot,
          labels = c("A.", "B."), 
          ncol = 1, nrow = 2)
```
...
```{r case_studies}
cs_educators = educatordf %>% select(starts_with("looked_at_")) %>%
  select(-looked_at_none) %>% na_if("NA") 
cs_students = studentdf %>% select(starts_with("looked_at_")) %>% na_if("NA") 
cs_self_learners = self_learnerdf %>% select(starts_with("looked_at_")) %>% na_if("NA") 
looked_at_cs = bind_rows(cs_educators, cs_students, cs_self_learners)

looked_at_cs_all = looked_at_cs %>% mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "case_study", values_to = "count") %>%
  mutate(percent = 100*count/nrow(looked_at_cs)) %>%
  mutate(responder_category = "All")


looked_at_cs_edu = cs_educators %>% mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>%
  pivot_longer(cols = everything(), names_to = "case_study", values_to = "count") %>%
  mutate(percent = 100*count/nrow(cs_educators)) %>%
  mutate(responder_category = "Educator")

looked_at_cs_stud = cs_students %>% mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>%
  pivot_longer(cols = everything(), names_to = "case_study", values_to = "count") %>%
  mutate(percent = 100*count/nrow(cs_students)) %>%
  mutate(responder_category = "Student")

looked_at_cs_sl = cs_self_learners %>% mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>%
  pivot_longer(cols = everything(), names_to = "case_study", values_to = "count") %>%
  mutate(percent = 100*count/nrow(cs_self_learners)) %>%
  mutate(responder_category = "Self-learner")

looked_at_cs_counts = bind_rows(looked_at_cs_all, looked_at_cs_edu, looked_at_cs_stud,
                                looked_at_cs_sl) %>%
  mutate(case_study = case_when(
    str_detect(case_study, pattern = "obesity") == TRUE ~ "Obesity",
    str_detect(case_study, pattern = "diet") == TRUE ~ "Diet",
    str_detect(case_study, pattern = "school_shootings") == TRUE ~ "School Shootings",
    str_detect(case_study, pattern = "RTC_wrangling") == TRUE ~ "RTC Wrangling",
    str_detect(case_study, pattern = "RTC_analysis") == TRUE ~ "RTC Analysis",
    str_detect(case_study, pattern = "air_pollution") == TRUE ~ "Air Pollution",
    str_detect(case_study, pattern = "co2_emissions") == TRUE ~ "CO2 Emissions",
    str_detect(case_study, pattern = "youth_disconnection") == TRUE ~ "Youth Disconnection",
    str_detect(case_study, pattern = "mental_health") == TRUE ~ "Youth Mental Health",
    str_detect(case_study, pattern = "opioids") == TRUE ~ "Opioids",
    str_detect(case_study, pattern = "vaping") == TRUE ~ "Vaping"
    )
  ) %>% mutate(percent = round(percent))

ggplot(looked_at_cs_counts, aes(x = reorder(case_study, -percent), y = percent, fill = responder_category)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Case Study", y = "Percent (%)", 
       title = "Which Case Studies did Participants Look at?") +
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  guides(fill = guide_legend(title = "Category")) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + ylim(c(0,45))
```


```{r recommend}
rec_educators = educatordf %>% select(recommend)
rec_students = studentdf %>% select(recommend)
rec_self_learners = self_learnerdf %>% select(recommend) %>% drop_na()
recommend = bind_rows(rec_educators, rec_students, rec_self_learners) %>% drop_na()

rec3 = round(100*sum(recommend == 3)/nrow(recommend), 1)
rec4 = round(100*sum(recommend == 4)/nrow(recommend), 1)
rec5 = round(100*sum(recommend == 5)/nrow(recommend), 1)

recdf = tibble(response = c("3", "4", "5"), percent = c(rec3, rec4, rec5), category = "All")

edurec3 = round(100*sum(rec_educators == 3)/nrow(rec_educators), 1)
edurec4 = round(100*sum(rec_educators == 4)/nrow(rec_educators), 1)
edurec5 = round(100*sum(rec_educators == 5)/nrow(rec_educators), 1)

edurecdf = tibble(response = c("3", "4", "5"), 
                  percent = c(edurec3, edurec4, edurec5), category = "Educator")

studrec3 = round(100*sum(rec_students == 3)/nrow(rec_students), 1)
studrec4 = round(100*sum(rec_students == 4)/nrow(rec_students), 1)
studrec5 = round(100*sum(rec_students == 5)/nrow(rec_students), 1)

studrecdf = tibble(response = c("3", "4", "5"), 
                  percent = c(studrec3, studrec4, studrec5), category = "Student")

slrec3 = round(100*sum(rec_self_learners == 3)/nrow(rec_self_learners), 1)
slrec4 = round(100*sum(rec_self_learners == 4)/nrow(rec_self_learners), 1)
slrec5 = round(100*sum(rec_self_learners == 5)/nrow(rec_self_learners), 1)

slrecdf = tibble(response = c("3", "4", "5"), 
                  percent = c(slrec3, slrec4, slrec5), category = "Self-learner")

rec_totals = bind_rows(recdf, edurecdf, studrecdf, slrecdf)

ggplot(rec_totals, aes(x = factor(response, levels = c("1","2","3","4","5")), y = percent, fill = category)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Rank", y = "Percent of Responses (%)") + 
  guides(fill = guide_legend(title = "Category")) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + ylim(c(0,80)) + scale_x_discrete(drop = FALSE)
```

```{r parts_interested}
parts_interest_sum = educatordf %>% select(starts_with("interest_")) %>% 
  select(contains(c("data", "part", "full", "combination"))) %>% 
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "parts", values_to = "count") %>%
  mutate(percent = 100*count/nrow(educatordf)) %>%
    mutate(parts = case_when(
    str_detect(parts, pattern = "data") == TRUE ~ "Data Only",
    str_detect(parts, pattern = "part") == TRUE ~ "Part of Case Study",
    str_detect(parts, pattern = "full") == TRUE ~ "Full Case Study",
    str_detect(parts, pattern = "combination") == TRUE ~ "Combination"
    )
  )

partsplot = ggplot(parts_interest_sum, aes(x = reorder(parts, -percent), y = percent, fill = parts)) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Material", y = "Percent of Educators (%)", 
       title = "Case Study Materials of Interest for Educators") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,75))
...
```{r sections_interested}
sect_interest_sum = educatordf %>% select(ends_with("_interest")) %>%
  select(-c(ocs_interest, materials_interest)) %>% na_if("NA") %>% drop_na() %>%
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "sections", values_to = "count") %>%
  mutate(percent = 100*count/nrow(educatordf)) %>%
    mutate(sections = case_when(
    str_detect(sections, pattern = "motivation") == TRUE ~ "Motivation/Context",
    str_detect(sections, pattern = "import") == TRUE ~ "Import",
    str_detect(sections, pattern = "wrangling") == TRUE ~ "Wrangling",
    str_detect(sections, pattern = "exploration") == TRUE ~ "Exploration",
    str_detect(sections, pattern = "visualization") == TRUE ~ "Visualization",
    str_detect(sections, pattern = "analysis") == TRUE ~ "Analysis",
    str_detect(sections, pattern = "data") == TRUE ~ "Data",
    str_detect(sections, pattern = "all_") == TRUE ~ "All Sections"
    )
  )

sectionsplot = ggplot(sect_interest_sum, aes(x = reorder(sections, -percent), y = percent, fill = sections)) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Section", y = "Percent of Educators (%)", 
       title = "Case Study Sections of Interest for Educators") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,75))
...
```{r partsinterestplots}
ggarrange(partsplot, sectionsplot,
          labels = c("A.", "B."), 
          ncol = 1, nrow = 2)
```

```{r refer_back}
refer_back_stud = studentdf %>% select(refer_back) %>% drop_na()
refer_back_sl = self_learnerdf %>% select(refer_back) %>% drop_na()
refer_back = bind_rows(refer_back_stud, refer_back_sl)
referdf = refer_back %>% group_by(refer_back) %>% summarise(count = n()) %>% 
  mutate(percent = round(100*count/sum(count), 2))

referplot = ggplot(referdf, aes(x = factor(refer_back, levels = c("1","2","3","4","5")), y = percent, fill = as.factor(refer_back))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank", y = "Percent of Responses (%)", 
       title = "Refer Back") + 
  guides(fill = "none") + scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,80)) + scale_x_discrete(drop = FALSE)
```


```{r enjoyment}
enjoy_stud = studentdf %>% select(enjoyment) %>% drop_na()
enjoy_sl = self_learnerdf %>% select(enjoyment) %>% drop_na()
enjoy = bind_rows(enjoy_stud, enjoy_sl)
enjoydf = enjoy %>% group_by(enjoyment) %>% 
  summarise(count = n()) %>% 
  mutate(percent = round(100*count/sum(count),0))

enjoyplot = ggplot(enjoydf, aes(x =factor(enjoyment, levels = c("1","2","3","4","5")), y = percent, fill = as_factor(enjoyment))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank", y = "Percent of Responses (%)", 
       title = "Enjoyment") + 
  guides(fill = "none") + scale_y_continuous(labels = function(x) paste0(x, "%")) + 
...
```{r usefulness}
useful_stud = studentdf %>% select(usefulness) %>% drop_na()
useful_sl = self_learnerdf %>% select(usefulness) %>% drop_na()
useful = bind_rows(useful_stud, useful_sl)
usefuldf = useful %>% group_by(usefulness) %>% 
  summarise(count = n()) %>% 
  mutate(percent = round(100*count/sum(count),0))

usefulplot = ggplot(usefuldf, aes(x = factor(usefulness, levels = c("1","2","3","4","5")), y = percent, fill = as_factor(usefulness))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank", y = "Percent of Responses (%)", 
       title = "Usefulness") + 
  guides(fill = "none") + scale_y_continuous(labels = function(x) paste0(x, "%")) + 
...
```{r stud_sl_review}
ggarrange(usefulplot, referplot + rremove("ylab") + 
            rremove("y.text"), enjoyplot + rremove("xlab"),
          labels = c("A.", "B.", "C."), 
          ncol = 2, nrow = 2)
```

```{r familiarity}
famil_stud = studentdf %>% select(starts_with("famil_")) %>% filter_all(any_vars(!is.na(.)))
famil_sl = self_learnerdf %>% select(starts_with("famil_")) %>% filter_all(any_vars(!is.na(.)))
familiarity = bind_rows(famil_stud, famil_sl)

famstats = familiarity %>%
  group_by(famil_stats) %>% summarise(count = n()) %>%
  mutate(percent = 100*count/sum(count)) %>% mutate(topic = "Statistics") %>%
  rename(response = famil_stats) %>% mutate(response = as.factor(response))

famPH = familiarity %>%
  group_by(famil_PH) %>% summarise(count = n()) %>% 
  mutate(percent = 100*count/sum(count)) %>% mutate(topic = "Public Health") %>%
  rename(response = famil_PH) %>% mutate(response = as.factor(response))

famDS = familiarity %>%
  group_by(famil_DS) %>% summarise(count = n()) %>% 
  mutate(percent = 100*count/sum(count)) %>% mutate(topic = "Data Science") %>%
  rename(response = famil_DS) %>% mutate(response = as.factor(response))

famtverse = familiarity %>%
  group_by(famil_tidyverse) %>% summarise(count = n()) %>% 
  mutate(percent = 100*count/sum(count)) %>% mutate(topic = "Tidyverse") %>%
  rename(response = famil_tidyverse) %>% mutate(response = as.factor(response))

famnonR = familiarity %>%
  group_by(famil_nonR) %>% summarise(count = n()) %>% 
  mutate(percent = 100*count/sum(count)) %>% mutate(topic = "Other Programming") %>%
  rename(response = famil_nonR) %>% mutate(response = as.factor(response))

famR = familiarity %>%
  group_by(famil_R) %>% summarise(count = n()) %>% 
  mutate(percent = 100*count/sum(count)) %>% mutate(topic = "R Programming") %>%
  rename(response = famil_R) %>% mutate(response = as.factor(response))

famildf1 = bind_rows(famstats, famPH, famDS)
famildf2 = bind_rows(famtverse, famnonR, famR) %>% 
  mutate(response = fct_inseq(response))

ggplot(famildf1, aes(x = factor(response, levels = c("1","2","3","4"), 
                                                 labels = c("Never heard\nof the topic", 
                                                            "Had heard briefly\nabout the topic",
                                                            "Had an understanding\nabout the topic\nbut not everything\ncovered",
                                                            "Already knew all\nor most of\nthe material")), 
                                      y = percent, fill = topic)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Response", y = "Percent of Responses (%)", 
       title = "Student and Self-Learner Familiarity with Case Study Topics") + 
  guides(fill = guide_legend(title = "Topic")) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + ylim(c(0,75)) + scale_x_discrete(drop = FALSE)
```

```{r familiarity2}
ggplot(drop_na(famildf2), aes(x = response, y = percent, fill = topic)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Rank", y = "Percent of Responses (%)", 
       title = "Student and Self-Learner Familiarity with Case Study Skills") + 
  guides(fill = guide_legend(title = "Skill")) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + ylim(c(0,75))
```

```{r familiarity_plots}
ggarrange(familtopicplot, familskillsplot,
          labels = c("A.", "B."), 
          ncol = 1, nrow = 2)
```

```{r stud_learning_interests}
learn_int_stud = studentdf %>% select(starts_with("learn_"))
learn_int_sl = self_learnerdf %>% select(starts_with("learn_"))

learn_int_studdf = learn_int_stud %>%
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "topics", values_to = "count") %>%
  mutate(percent = round(100*count/nrow(learn_int_stud), digits = 2)) %>%
    mutate(topics = case_when(
    str_detect(topics, pattern = "learn_stats") == TRUE ~ "Statistics",
    str_detect(topics, pattern = "learn_ds") == TRUE ~ "Data Science",
    str_detect(topics, pattern = "learn_PH") == TRUE ~ "Public Health",
    str_detect(topics, pattern = "_R") == TRUE ~ "R Programming",
    str_detect(topics, pattern = "_tidyverse") == TRUE ~ "Tidyverse",
    str_detect(topics, pattern = "_spfc_stats") == TRUE ~ "Specific\n Statistics\n Topic",
    str_detect(topics, pattern = "_spfc_ds") == TRUE ~ "Specific\n Data Science\n Topic",
    str_detect(topics, pattern = "_spfc_PH") == TRUE ~ "Specific\n Public Health\n Topic"
    )
  ) %>% mutate(ctg = "Student")

learn_int_sldf = learn_int_sl %>%
  mutate(across(, ~as.numeric(.))) %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "topics", values_to = "count") %>%
  mutate(percent = round(100*count/nrow(learn_int_sl), digits = 4)) %>%
    mutate(topics = case_when(
    str_detect(topics, pattern = "learn_stats") == TRUE ~ "Statistics",
    str_detect(topics, pattern = "learn_ds") == TRUE ~ "Data Science",
    str_detect(topics, pattern = "learn_PH") == TRUE ~ "Public Health",
  ...
  ) %>% mutate(ctg = "Self-learner")

studintplot = ggplot(learn_int_studdf, aes(x = reorder(as_factor(topics), -percent), y = percent, fill = as_factor(topics))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Topic", y = "Percent of Responses (%)", 
       title = "Student Learning Interests") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(limit = c(0,25,50,75,100)) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,100))
```

```{r self_learn_interests}
slintplot = ggplot(learn_int_sldf, aes(x = reorder(as_factor(topics), -percent), y = percent, fill = as_factor(topics))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Topic", y = "Percent of Responses (%)", 
       title = "Self-learner Learning Interests") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
...
```{r learning_interests_figure}
ggarrange(studintplot, slintplot,
          labels = c("A.", "B."), 
          ncol = 1, nrow = 2)
```

```{r learned_new}
learn_new_stud = studentdf %>% select(starts_with("learned_new_"), 
                                      -c("learned_new_all", "learned_new_none"))
learn_new_sl = self_learnerdf %>% select(starts_with("learned_new_"), 
                                         -c("learned_new_all", "learned_new_none"))
learn_new = bind_rows(learn_new_stud, learn_new_sl) %>% na_if("NA") %>% drop_na() 
learn_newdf = learn_new %>% mutate(across(, ~as.numeric(.))) 
learn_newdf[3,] = learn_newdf[3,] + 1 # adding 1 to each for "all of the above" response
learn_newdf = learn_newdf %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "topics", values_to = "count") %>%
  mutate(percent = round(100*count/nrow(learn_new),2)) %>%
    mutate(topics = case_when(
    str_detect(topics, pattern = "new_stats") == TRUE ~ "Statistics",
    str_detect(topics, pattern = "Rstats") == TRUE ~ "RStats",
    str_detect(topics, pattern = "import") == TRUE ~ "Data Import",
    str_detect(topics, pattern = "viz") == TRUE ~ "Data Visualization",
    str_detect(topics, pattern = "wrang") == TRUE ~ "Data Wrangling",
    str_detect(topics, pattern = "comm") == TRUE ~ "Data Communication",
    str_detect(topics, pattern = "new_PH") == TRUE ~ "Public Health"
    )
  )

ggplot(learn_newdf, aes(x = reorder(as_factor(topics), -percent), y = percent, fill = as_factor(topics))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Topic", y = "Percent of Responses (%)", 
       title = "Students and Self-learners Learned Something New About:") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,80))
```
```{r how_found}
howfound = studentdf %>% select(how_found)
num_responses = nrow(howfound)
howfound %<>% count(how_found, name = "num_students") %>% 
  mutate(percent = round(100*num_students/num_responses,0),
         reason = case_when(
           str_detect(how_found, "assignment") == TRUE ~ "Assignment",
           TRUE ~ "Supplement")
         )

rplot = ggplot(howfound, aes(x = as_factor(reason), y = percent, 
                   fill = as_factor(reason))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Reason", y = "Percent of Students (%)", 
       title = "Students' Reasons for Case Study Use") + 
  guides(fill = "none") + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,65))
```

```{r student_type}
studtype = studentdf %>% select(student_type)
num_responses = nrow(studtype)
studtype %<>% count(student_type, name = "num_students") %>% 
  mutate(percent = round(100*num_students/num_responses,0),
         student_type = str_replace(student_type, "College", "Undergraduate")
         )

lplot = ggplot(studtype, aes(x = as_factor(student_type), y = percent, 
                   fill = as_factor(student_type))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Level", y = "Percent of Students (%)", 
       title = "Academic Level of Student Participants") + 
  guides(fill = "none") + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,65))
```
