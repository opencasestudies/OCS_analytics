...
ggplot(referdf, aes(x = as.factor(refer_back), y = percent, fill = as.factor(refer_back))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 5)", y = "Percent of Responses (%)", 
       title = "How likely are you to refer back to the case studies in the future?") + 
  guides(fill = "none") + scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,50))
```
