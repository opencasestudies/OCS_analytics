...
partsplot = ggplot(parts_interest_sum, aes(x = parts, y = percent, fill = parts)) +
  geom_col() + scale_fill_viridis_d() + 
...
sectionsplot = ggplot(sect_interest_sum, aes(x = sections, y = percent, fill = sections)) +
  geom_col() + scale_fill_viridis_d() + 
...
referplot = ggplot(referdf, aes(x = as.factor(refer_back), y = percent, fill = as.factor(refer_back))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 5)", y = "Percent of Responses (%)", 
       title = "Refer Back") + 
...
enjoyplot = ggplot(enjoydf, aes(x = as_factor(enjoyment), y = percent, fill = as_factor(enjoyment))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 5)", y = "Percent of Responses (%)", 
       title = "Enjoyment") + 
  guides(fill = "none") + scale_y_continuous(labels = function(x) paste0(x, "%")) + 
...
usefulplot = ggplot(usefuldf, aes(x = as_factor(usefulness), y = percent, fill = as_factor(usefulness))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 5)", y = "Percent of Responses (%)", 
       title = "Usefulness") + 
  guides(fill = "none") + scale_y_continuous(labels = function(x) paste0(x, "%")) + 
...
```{r familiarity}
famil_stud = studentdf %>% select(starts_with("famil_")) %>% drop_na()
famil_sl = self_learnerdf %>% select(starts_with("famil_")) %>% drop_na() 
...
familtopicplot = ggplot(famildf1, aes(x = response, y = percent, fill = topic)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 4)", y = "Percent of Responses (%)", 
...
```{r familiarity2}
familskillsplot = ggplot(famildf2, aes(x = response, y = percent, fill = topic)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 5)", y = "Percent of Responses (%)", 
...
ggplot(learn_intdf, aes(x = as_factor(topics), y = percent, fill = as_factor(topics))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Topic", y = "Percent of Responses (%)", 
       title = "Student and Self-learner Learning Interests") + 
  guides(fill = "none") + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + 
  geom_text(aes(label = paste0(percent, "%")), vjust = -.5) + ylim(c(0,100))
...
learn_newdf = learn_newdf %>%
  summarise_all(., ~ sum(.x, na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = "topics", values_to = "count") %>%
  mutate(percent = 100*count/nrow(learn_new)) %>%
    mutate(topics = case_when(
...
ggplot(learn_newdf, aes(x = as_factor(topics), y = percent, fill = as_factor(topics))) +
  geom_col() + scale_fill_viridis_d() + 
  labs(x = "Topic", y = "Percent of Responses (%)", 
