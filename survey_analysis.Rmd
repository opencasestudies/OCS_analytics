...
ggplot(consented, aes(x = as.factor(responder_category), fill = responder_category)) +
  geom_bar() + 
...
teach_int_sum = tibble(teach_stats = 
                         sum(as.numeric(educatordf$teach_stats)), 
                  teach_spfc_stats = 
                    sum(as.numeric(educatordf$teach_spfc_stats)),
                  teach_ds = 
                    sum(as.numeric(educatordf$teach_ds)),
                  teach_spfc_ds =
                    sum(as.numeric(educatordf$teach_spfc_ds)),
                  teach_R = 
                    sum(as.numeric(educatordf$teach_R)),
                  teach_tidyverse = 
                    sum(as.numeric(educatordf$teach_tidyverse)),
                  teach_PH = 
                    sum(as.numeric(educatordf$teach_PH)),
                  teach_spfc_PH = 
                    sum(as.numeric(educatordf$teach_spfc_PH))
                  )


ggplot(teach_int_sum, aes()) +
  geom_bar() + 
  scale_fill_manual(values = c("#5592CC", "#714986", "#4B9B81")) + 
