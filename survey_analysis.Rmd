...
ggplot(rec_totals, aes(x = response, y = percent, fill = category)) +
  geom_bar(position = "dodge", stat = "identity") + scale_fill_viridis_d() + 
  labs(x = "Rank (out of 5)", y = "Percent of Responses (%)", 
       title = "How likely are you to recommend the case studies to others?") + 
  guides(fill = guide_legend(title = "Category")) + 
  scale_y_continuous(labels = function(x) paste0(x, "%")) + ylim(c(0,80))
```
